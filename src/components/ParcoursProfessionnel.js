...
export const ParcoursProfessionnel = () => {
  const [activeLink, setActiveLink] = useState('parcours professionnel');
  const [loopNum, setLoopNum] = useState(0);
  const [isDeleting, setIsDeleting] = useState(false);
  const [text, setText] = useState('');
  const [delta, setDelta] = useState(300 - Math.random() * 100);
  const [index, setIndex] = useState(1);
  const toRotate = ["Parcours Professionnel"];
  const period = 2000;
  ...
  return (
    <section className="home" id="parcours-professionnel">
      <Container>
          ...
          <Col xs={12} md={6} xl={7}>
            <TrackVisibility>
              ...
              <div className={isVisible ? "animate__animated animate__fadeIn" : ""}>
                <h1>{`Page`} <span className="txt-rotate" dataPeriod="1000" data-rotate='[ "Parcours Professionnel" ]'><span className="wrap">{text}</span></span></h1>
                <p>
