...
export const DeveloppementProfessionnel = () => {
  const [activeLink, setActiveLink] = useState('Developpement Professionnel');
  const [loopNum, setLoopNum] = useState(0);
  const [isDeleting, setIsDeleting] = useState(false);
  const [text, setText] = useState('');
  const [delta, setDelta] = useState(300 - Math.random() * 100);
  const [index, setIndex] = useState(1);
  const toRotate = ["Developpement Professionnel"];
  const period = 2000;
  ...
  return (
    <section className="home" id="Developpement-Professionnel">
      <Container>
...
export default DeveloppementProfessionnel;